...
static ExecutionReport makeReport(const char *clientOrderId,
                                  const char *orderId,
...
std::vector<ExecutionReport> ExecutionHandler::buildFillReports(
    ...
    for (const Trade &t : trades) {
        ...
        reports.push_back(makeReport(
            t.passive.order.clientOrderId, t.passive.orderId,
            t.passive.order.instrument,    t.passive.order.side,
            execPrice, t.execQty, passiveStatus));

        reports.push_back(makeReport(
            aggressor.clientOrderId, aggressorOrderId,
            aggressor.instrument,    aggressor.side,
            execPrice, t.execQty, aggressorStatus));
    }
...
ExecutionReport ExecutionHandler::buildNewReport(const BookEntry &entry) {
    return makeReport(
        entry.order.clientOrderId, entry.orderId,
        entry.order.instrument,    entry.order.side,
        entry.order.price, entry.remainingQty, Status::New);
}
