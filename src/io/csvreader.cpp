...
CSVReader::splitCommaSeparated(const std::string &line) {
...
bool CSVReader::isHeaderRow(const std::string &line) {
  ...
  return tokens[0] == "Client Order ID" && tokens[1] == "Instrument" &&
         tokens[2] == "Side" && tokens[3] == "Quantity" && tokens[4] == "Price";
}
...
ParseResult CSVReader::buildOrder(const RawOrderRow &raw) {
  ...
  auto resultParse = std::from_chars(
      raw.side.data(), raw.side.data() + raw.side.size(), sideValue);

  if (resultParse.ec != std::errc() ||
      resultParse.ptr != raw.side.data() + raw.side.size()) {
  ...
  auto quantityParse =
      std::from_chars(raw.quantity.data(),
  ...
  std::snprintf(result.order.clientOrderId, sizeof(result.order.clientOrderId),
                "%s", raw.clientOrderId.c_str());
